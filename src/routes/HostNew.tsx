...
function Step2AI({ d, setD }: { d: Draft; setD: (n: Draft) => void }) {
  const [result, setResult] = useState<ReturnType<typeof detectAllergens> | null>(null);
  const [running, setRunning] = useState(false);

  const run = () => {
    setRunning(true);
    setTimeout(() => {
      const r = detectAllergens(`${d.title} ${d.description}`);
      setResult(r);
      // Merge AI flags with any manual edits
      ...
      {!running && result && (
        <>
          <div className="rounded-2xl border-2 border-amber/60 bg-amber/15 p-4">
            <div className="font-display font-bold text-amber-ink mb-2">
              ⚠ We detected
            </div>
          ...
          {result.suggestedDietary.length > 0 && (
